...
│  │ device.nng  │ │ SnakeOil     │ │  boot.py               ││
│  │ brand.txt   │ │ Envelope     │ │  register.py           ││
│  │ APPCID      │ │ igo-binary   │ │  market.py             ││
│  │ model match │ │              │ │  catalog.py            ││
│  └─────────────┘ └──────────────┘ └────────────────────────┘│
│  ┌─────────────┐ ┌──────────────┐ ┌────────────────────────┐│
│  │ swid.py     │ │ download.py  │ │ installer.py           ││
│  │ MD5+SPEEDx  │ │ Cache        │ │ USB layout             ││
│  │ CK-XXXX fmt │ │ Resume       │ │ .lyc/.stm/.md5         ││
│  │ drive serial│ │ MD5 verify   │ │ update_checksum        ││
│  └─────────────┘ └──────────────┘ └────────────────────────┘│
│  ┌─────────────┐ ┌──────────────┐                           │
│  │fingerprint  │ │ crypto.py    │                           │
│  │  .py        │ │ SnakeOil     │                           │
│  │ Read/build  │ │ Blowfish     │                           │
│  │ Send to srv │ │ NNGE / MD5   │                           │
│  └─────────────┘ └──────────────┘                           │
└──────────────────────────────────────────────────────────────┘
```
