...
### Encoder — Metacharacter Mapping (from Unicorn Trace)

The encoder maps control records back to regex metacharacters:

| Control Record | Metachar | Hex | Description |
|---------------|----------|-----|-------------|
| 0x80090000 | `^` | 0x5E | Separator/anchor |
| 0x80160000 | `$` | 0x24 | Line end |
| 0x80010000 | `\|` | 0x7C | Alternation |
| 0x80330000 | `+` | 0x2B | Road segment 33 |
| 0x80170000 | `.` | 0x2E | Wildcard/marker |
| 0x800A0000 | `[` | 0x5B | Character class/attribute |
| 0x80180000 | `\` + data | 0x5C | Escape sequence |
| 0x80000000 | LF | 0x0A | End of line |

Data records are encoded as UTF-8-like varints (pass-through).

**Roundtrip accuracy:** 100% for data records. Control records round-trip
correctly for the simple metacharacter types. Complex control records
(0x80360000, 0x80080000, etc.) are generated by multi-varint pattern
sequences that require exact byte reproduction.

### Tools Summary (24 tools)
